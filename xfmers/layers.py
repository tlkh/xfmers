...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, d_model, num_heads, causal=False, name="MultiHeadAttention"):
        """
        Initialize MHA
        Args:
    ...
    def call(self, inputs):
        ...
        scaled_attention = ops.scaled_dot_product_attention(query, key, value,
                                                            mask)
...
class TransformerLayer(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, ff_units, d_model, num_heads, dropout, causal=False, activation="relu", name="TransformerLayer"):
        """
        ...
        self.mha = MultiHeadAttention(self.d_model, self.num_heads, causal=self.causal, name="MultiHeadAttention")
        ...
        self.ffn_2 = tf.keras.layers.Activation(self.activation)
        self.ffn_3 = tf.keras.layers.Dense(units=self.d_model)
    ...
    def call(self, inputs):
        ...
        outputs = self.ffn_3(self.ffn_2(self.ffn_1(attention)))
...
class TransformerStack(tf.keras.layers.Layer):
    """
    Build a stack of Transformer layers as Encoder/Decoder
    """
    def __init__(self, layers, ff_units, d_model, num_heads, dropout, causal=False, activation="relu", weight_sharing=False, name="TransformerStack"):
        """
        ...
        if self.weight_sharing:
            ...
            self.xfmer_layers = [self.xfmer_layer for i in range(self.layers)]
        else:
            ...
            self.xfmer_layers = [
                TransformerLayer(ff_units=self.ff_units,
                                 ...
                                 causal=self.causal,
                                 activation=self.activation,
                                 name=self.name+"_Layer_"+str(i+1)) for i in range(self.layers)
            ]
...
class TokenPosEmbedding(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, d_vocab, d_model, pos_length=512, scale=1, name="TokenPosEmbedding"):
        """
        ...
        Args:
            ...
            scale: (bool) Apply scaling of sqrt(d_model) to token embeddings. Defaults to True
        Inputs:
...
class PaddingMaskGenerator(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        outputs = ops.create_padding_mask(inputs)
        return outputs
...
class LMHead(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        outputs = self.dense_softmax(inputs[:,-1])
        return outputs
